...
function trimify(html, trim) {
	for (var e = 0; e < trim.length; e++) {
		var leading_whitespace = new RegExp(`(<${trim[e]}[^>]*>)\\s+`, "g");
		var trailing_whitespace = new RegExp(`\\s+(</${trim[e]}>)`, "g");
		html = html
...
function process(html, step) {
	...
	convert.line.forEach(function (source, index) {
		html = html
			...
			.replace(`[#-# : ${index} : ${source} : #-#]`, function (match) {
				...
				var offset = indents.length - subtrahend;
	...
	var lead_newline_check = html.substring(0, 1);
	var tail_newline_check = html.substring(html.length - 1);
	if (lead_newline_check === "\n") html = html.substring(1, html.length);
	if (tail_newline_check === "\n") html = html.substring(0, html.length - 1);
	return html;
}

window.prettifyHTML = function (html) {
	if (!isHtml(html)) return html;
	strict = CONFIG.strict;
	var ignore = CONFIG.ignore.length > 0;
	trim = CONFIG.trim;
	if (ignore) {
